...
def print_threshold_recommendations(ratios):
    ...
    # Отделяем "шум" от "смен слайдов" по квантилям.
    # Предполагаем: смен слайдов ~10-20, шума — сотни.
    # Значит, смены — это верхние 1-3% значений.
    ...
    if mx < 0.01:
        ...
        # Хорошая ситуация: смены слайдов чётко выделяются на фоне шума
...
def extract_frame(video_path, timestamp, output_dir):
    ...
    # Явно указываем в имени: часы-минуты-секунды
    output_filename = os.path.join(output_dir, f"slide_HH{ts_str[0:2]}-MM{ts_str[3:5]}-SS{ts_str[6:8]}.png")
...
Примеры:
  ...
  python extract_slides.py 05.mp4 slides --sample-interval 0.5
    ...
    print(f"\nИзвлечение кадров с задержкой {args.capture_delay} сек в папку '{args.output_dir}'...")
